...
internals.Connection.prototype.isReady = function () {
    ...
    // For debug purpose
    if (this.alwaysReady) return true;
...
internals.Connection.prototype.set = async function (key, value, ttl) {
    ...
    await this._run(
        () => this.primary.get(key, value, ttl),
        () => this.secondary.get(key, value, ttl)
    );
};
...
/**
 * This method is only use for debugging purpose.
 * It will push {who: 'primary'} in the primary and
 * and {who: 'secondary'} in the secondary
 * It should be called after engines are started
 *
 * @param {Number} ttl
 ...
 * @private
 */
internals.Connection.prototype.setDebugFlags = async function (ttl) {
    if (!this.debug) {
        throw new Error('setDebugFlags is only available in debug mode');
    }
    await Promise.all([
        this.primary.set('who', 'primary', ttl),
        this.secondary.set('who', 'secondary', ttl)
    ]);
};